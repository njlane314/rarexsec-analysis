...
public:
  ...
  void processSystematics(VariableResult &result) {
    for (const auto &strategy : systematic_strategies_) {
      ...
      result.covariance_matrices_[key] =
          strategy->computeCovariance(result, systematic_futures_);
    }
    ...
    if (n_bins > 0) {
      ...
      for (const auto &[name, cov] : result.covariance_matrices_) {
        if (cov.GetNrows() == n_bins) {
          ...
          log::warn("SystematicsProcessor", "Skipping systematic", name.str(),
                    "due to incompatible matrix size (", cov.GetNrows(), "x",
                    cov.GetNcols(), "vs expected", n_bins, "x", n_bins, ")");
        }
      }
