...
public:
  ...
  void onInitialisation(AnalysisDefinition &def,
    ...
    if (!config_.contains("variables"))
      log::fatal("VariablesPlugin::onInitialisation",
    ...
    for (auto const &var_cfg : config_.at("variables")) {
      ...
      if (bins_cfg.is_string() && bins_cfg.get<std::string>() == "dynamic") {
        ...
        double domain_min =
            bins_cfg.value("min", -std::numeric_limits<double>::infinity());
        double domain_max =
            bins_cfg.value("max", std::numeric_limits<double>::infinity());
        bool include_oob_bins = bins_cfg.value("include_oob_bins", true);
        double resolution = bins_cfg.value("resolution", 0.0);
        std::string strat_mode =
            bins_cfg.value("strategy", std::string("equal_weight"));
        static const std::unordered_map<std::string, DynamicBinningStrategy>
            strategy_map = {
        ...
        BinningDefinition placeholder_bins({domain_min, domain_max}, branch,
                                           label, {}, strat);
        def.addVariable(name, branch, label, placeholder_bins, strat, true,
                        include_oob_bins, strategy, resolution);
      } else {
      ...
      if (var_cfg.contains("regions")) {
        ...
        log::warn("VariablesPlugin::onInitialisation",
                  "Variable '{}' has no 'regions' field and will not be "
                  "attached to any regions",
                  name);
      }
    }
  }
...
ANALYSIS_REGISTER_PLUGIN(analysis::IAnalysisPlugin,
                         analysis::AnalysisDataLoader, "VariablesPlugin",
                         analysis::VariablesPlugin)

#ifdef BUILD_PLUGIN
extern "C" analysis::IAnalysisPlugin *
