...
for r in 1 2 3 4 5; do
  ...
  hdr "BNB"
  read -r EXT Gate2 Cnt TorA TorB CntW TorAW TorBW < <(bnb_totals_sql "$S" "$E")
  row "$r" "TOTAL" "$EXT" "$Gate2" "$Cnt" "$TorA" "$TorB" "$CntW" "$TorAW" "$TorBW"

  echo
  hdr "NuMI"
  read -r EXT1 Gate1 CntN Tor101 Tortgt CntNW Tor101W TortgtW < <(numi_totals_sql "$S" "$E")
  row "$r" "TOTAL" "$EXT1" "$Gate1" "$CntN" "$Tor101" "$Tortgt" "$CntNW" "$Tor101W" "$TortgtW"

  if $have_numi_v4; then
    read -r cntF tor101F tortgtF < <(numi_horns_sql "$S" "$E" "FHC")
    row "$r" "FHC"  0 0 0 0 0 "$cntF" "$tor101F" "$tortgtF"
    read -r cntR tor101R tortgtR < <(numi_horns_sql "$S" "$E" "RHC")
