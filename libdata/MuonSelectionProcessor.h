...
public:
    ROOT::RDF::RNode process(ROOT::RDF::RNode df, SampleOrigin st) const override {
        ...
        auto avg_df = df.Define(
            "trk_rr_dedx_avg",
            [](const ROOT::RVec<float>& u,
               ...
               const ROOT::RVec<float>& y) {
                ...
                for (size_t i = 0; i < u.size(); ++i) {
                    int count = 0;
                    float sum = 0;
                    if (u[i] > 0) { sum += u[i]; ++count; }
                    if (v[i] > 0) { sum += v[i]; ++count; }
                    if (y[i] > 0) { sum += y[i]; ++count; }
                    avg[i] = count ? sum / count : -1;
                }
                return avg;
            },
            {"track_trunk_rr_dedx_u",
             "track_trunk_rr_dedx_v",
        ...
        auto count_df = mask_df.Define(
            "n_muons",
            "ROOT::VecOps::Sum(muon_mask)"
        );

        auto has_df = count_df.Define(
            "has_muon",
            "n_muons > 0"
        );
