...
class IHistogramStratifier {
  public:
    ...
    stratifyHist(ROOT::RDF::RNode dataframe, const BinningDefinition &binning,
                 const ROOT::RDF::TH1DModel &hist_model,
                 const std::string &weight_column) const {
        analysis::log::info("IHistogramStratifier::stratifyHist",
                            "Starting stratifiying histograms...");
        std::unordered_map<StratumKey, ROOT::RDF::RResultPtr<TH1D>>
            strat_futurs;
        ...
        for (const auto &key : this->getRegistryKeys()) {
            ...
            std::replace(stratum_key_str.begin(), stratum_key_str.end(), '-',
                         'n');
            std::string filter_col_name =
                "pass_" + getSchemeName() + "_" + stratum_key_str;

            df_with_filters = this->defineFilterColumn(
                df_with_filters, std::stoi(key.str()), filter_col_name);
            strat_futurs[key] =
                df_with_filters.Filter(filter_col_name)
  ...
  protected:
    ...
    std::vector<StratumKey> getRegistryKeys() const {
        return this->getRegistry().getAllStratumKeysForScheme(
            this->getSchemeName());
    }
};
