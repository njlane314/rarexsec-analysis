...
static void runPlotting(const nlohmann::json &config_data, const nlohmann::json &config_plot, const analysis::AnalysisResult &res) {
    ...
    for (auto const &[beam, config_run] : config_data.at("run_configurations").items()) {
        ...
        loaders.emplace(beam, std::make_unique<analysis::AnalysisDataLoader>(rc_reg, ev_reg, beam, periods, ntuple_base_directory, true));
    }
    ...
    auto beam_results = [&res] {
        ...
        for (auto &[k, v] : m) v.build();
        return m;
    }();

    for (auto &[beam, loader] : loaders) {
        analysis::AnalysisPluginManager manager;
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        analysis::log::fatal("plot::main", "Invocation error. Expected:", argv[0], "<config.json> <plugins.json> <input.root>");
        return 1;
    }
