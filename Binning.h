...
public:
    ...
    Binning(TString var, const std::vector<double>& edges, TString lbl = "",
        ...
        if (variable_tex.IsNull()) variable_tex = variable;
        if (label.IsNull()) label = variable;
        if (bin_edges.size() < 2) {
    ...
    static Binning fromConfig(TString var, int nBins, std::pair<double, double> range,
    ...
    Binning& setLabel(const TString& lbl) {
        label = lbl;
        return *this;
    }
};
