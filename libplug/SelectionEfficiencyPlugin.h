...
class SelectionEfficiencyPlugin : public IAnalysisPlugin {
  public:
    ...
    explicit SelectionEfficiencyPlugin(const nlohmann::json &cfg) {
        if (!cfg.contains("efficiency_plots") ||
            !cfg.at("efficiency_plots").is_array()) {
            throw std::runtime_error(
                "SelectionEfficiencyPlugin missing efficiency_plots");
        }
    ...
    void onInitialisation(AnalysisDefinition &def,
        ...
        for (auto &pc : plots_) {
            try {
                ...
                log::error("SelectionEfficiencyPlugin::onInitialisation",
                           e.what());
            }
        }
    }
    ...
    void onFinalisation(const RegionAnalysisMap &) override {
        if (!loader_) {
            log::error("SelectionEfficiencyPlugin::onFinalisation",
        ...
        for (const auto &pc : plots_) {
            ...
            stage_labels.insert(stage_labels.end(), pc.clauses.begin(),
                                pc.clauses.end());
            ...
            for (const auto &c : counts) {
                ...
                double eff_err =
                    (Neff0 > 0) ? std::sqrt(eff * (1.0 - eff) / Neff0) : 0.0;
                double pur = c.tot > 0 ? c.sig / c.tot : 0.0;
                double Neff_tot =
                    c.tot_w2 > 0 ? (c.tot * c.tot) / c.tot_w2 : 0.0;
                double pur_err = (Neff_tot > 0)
                                     ? std::sqrt(pur * (1.0 - pur) / Neff_tot)
                                     : 0.0;
