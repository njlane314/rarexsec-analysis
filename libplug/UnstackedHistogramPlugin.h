#ifndef UNSTACKEDHISTOGRAMPLUGIN_H
#define UNSTACKEDHISTOGRAMPLUGIN_H
...
public:
    ...
    void onInitialisation(AnalysisDefinition& def, const SelectionRegistry&) override {
        for (auto const& pc : plots_) {
            if (!pc.selection_cuts) continue;
            RegionKey rkey{pc.region};
            if (region_cuts_.count(rkey)) continue;
            try {
                const auto& sel_str = def.region(rkey).selection().str();
                parseSelectionCuts(rkey, sel_str);
            } catch (const std::exception& e) {
                log::error("UnstackedHistogramPlugin::onInitialisation",
                           "Could not parse selection for region",
                           rkey.str(), e.what());
            }
        }
    }
    ...
    void onFinalisation(const RegionAnalysisMap& region_map) override {
        ...
        for (auto const& pc : plots_) {
            ...
            if (it == region_map.end()) {
                log::error("UnstackedHistogramPlugin::onFinalisation", "Could not find analysis region for key:", rkey.str());
                continue;
            }

            VariableKey vkey{pc.variable};
            if (!it->second.hasFinalVariable(vkey)) {
                log::error("UnstackedHistogramPlugin::onFinalisation", "Could not find variable", vkey.str(), "in region", rkey.str());
                continue;
            }
            ...
            UnstackedHistogramPlot plot(
                "unstack_" + pc.variable + "_" + pc.region,
                variable_result,
                region_analysis,
                pc.category_column,
                pc.output_directory,
