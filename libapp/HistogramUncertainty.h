...
public:
    ...
    friend HistogramUncertainty operator*(double s, const HistogramUncertainty& h) {
        return h * s;
    }
};
...
inline HistogramUncertainty HistogramUncertainty::operator+(const HistogramUncertainty& o) const {
    ...
    if (shifts.rows() != o.shifts.rows()) {
        log::error("HistogramUncertainty::operator+", "Shifts matrix dimension mismatch: ",
                   shifts.rows(), " vs ", o.shifts.rows());
...
inline HistogramUncertainty HistogramUncertainty::operator*(const HistogramUncertainty& o) const {
    if (size() <= 0 || o.size() <= 0) return HistogramUncertainty();
    if (size() != o.size()) {
        log::fatal("HistogramUncertainty::operator*",
