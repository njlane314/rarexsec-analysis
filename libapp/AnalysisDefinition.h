...
public:
    ...
    AnalysisDefinition &
    addVariable(const std::string &key, const std::string &expr,
                const std::string &lbl, const BinningDefinition &bdef,
                const std::string &strat, bool is_dynamic = false,
    ...
    AnalysisDefinition &addRegion(const std::string &key,
        ...
        region_analyses_.emplace(
                region_key, this->makeRegionAnalysis(
    ...
    AnalysisDefinition &addRegionExpr(const std::string &key,
        ...
        region_selections_.emplace(region_key,
                                   SelectionQuery(std::move(raw_expr)));
    ...
    std::vector<VariableHandle> variables() const {
        ...
        for (const auto &entry : variable_expressions_)
            result.emplace_back(entry.first, variable_expressions_,
                                variable_labels_, variable_binning_,
...
private:
    ...
    makeRegionAnalysis(const RegionKey &key, const std::string &label,
                       double pot, bool blinded, std::string beam_config,
                       std::vector<std::string> runs) {
        return std::make_unique<RegionAnalysis>(key, label, pot, blinded,
