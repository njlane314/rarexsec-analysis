...
class BinnedHistogram : public TNamed,
                        public TH1DStorage,
                        private TH1DRenderer {
public:
  ...
  BinnedHistogram operator*(const BinnedHistogram &o) const {
    if (this->getNumberOfBins() <= 0 || o.getNumberOfBins() <= 0)
      return BinnedHistogram();
    if (this->getNumberOfBins() != o.getNumberOfBins()) {
      log::fatal(
          "BinnedHistogram::operator*",
          "Attempting to multiply histograms with different numbers of bins.");
    }
    auto tmp = *this;
  ...
  BinnedHistogram operator+(const BinnedHistogram &o) const {
    log::debug("BinnedHistogram::operator+", "Adding '", o.GetName(), "' (",
               o.getNumberOfBins(), " bins, shifts ", o.shifts.cols(), ") to '",
               this->GetName(), "' (", this->getNumberOfBins(),
               " bins, shifts ", this->shifts.cols(), ")");

    if (this->getNumberOfBins() <= 0)
      return o;
    if (o.getNumberOfBins() <= 0)
      return *this;

    if (this->getNumberOfBins() != o.getNumberOfBins()) {
      log::debug("BinnedHistogram::operator+", "Adding '", o.GetName(), "' (",
                 o.getNumberOfBins(), " bins, shifts ", o.shifts.cols(),
                 ") to '", this->GetName(), "' (", this->getNumberOfBins(),
                 " bins, shifts ", this->shifts.cols(), ")");
    }
    ...
    if (tmp.shifts.rows() != o.shifts.rows()) {
      log::error("BinnedHistogram::operator+",
                 "Shifts matrix dimension mismatch: ", tmp.shifts.rows(),
                 " vs ", o.shifts.rows());
      return BinnedHistogram();
    }
    int c1 = shifts.cols();
  ...
  BinnedHistogram operator-(const BinnedHistogram &o) const {
    ...
    if (this->getNumberOfBins() != o.getNumberOfBins()) {
      log::fatal(
          "BinnedHistogram::operator-",
          "Attempting to subtract histograms with different numbers of bins.");
    }
    auto tmp = *this;
    for (int i = 0; i < getNumberOfBins(); ++i) {
      tmp.counts[i] -= o.counts[i];
