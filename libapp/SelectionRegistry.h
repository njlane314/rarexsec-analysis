...
private:
  Selection makeSelection(const SelectionRule &r) const {
    ...
    return std::accumulate(
        std::next(r.clauses.begin()), r.clauses.end(),
        Selection(r.clauses.front()),
        [](Selection a, const std::string &b) { return a && Selection(b); });
  }

  void registerDefaults() {
    rules_.emplace("QUALITY",
                   SelectionRule{"Quality Preselection", {"quality_event"}});
    rules_.emplace("NUMU_CC", SelectionRule{"NuMu CC Selection",
