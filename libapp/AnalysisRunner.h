...
public:
  ...
  AnalysisRegionMap run() {
    ...
    for (const auto &var_handle : analysis_definition_.variables()) {
      if (analysis_definition_.isDynamic(var_handle.key_)) {
        ...
        BinningDefinition new_bins = DynamicBinningCalculator::calculate(
            mc_nodes, var_handle.binning(), "nominal_event_weight", 50.0);
    ...
    for (const auto &region_handle : regions) {
      ++region_index;
      analysis::log::info("AnalysisRunner::run", "Engaging region protocol (",
      ...
      for (auto &[sample_key, sample_def] : sample_frames) {
        ...
        analysis::log::info("AnalysisRunner::run", "→ Conditioning sample (",
        ...
        for (auto &[variation_type, variation_node] :
          ...
          variation_datasets.emplace(
              variation_type,
              AnalysisDataset{
                  sample_def.sample_origin_, AnalysisRole::kVariation,
                  variation_node.Filter(region_handle.selection().str())});
        }

        AnalysisDataset nominal_dataset{sample_def.sample_origin_,
                                        AnalysisRole::kNominal, region_df};
        SampleEnsemble ensemble{std::move(nominal_dataset),
      ...
      for (const auto &var_key : vars) {
        ...
        for (auto &[sample_key, rnode] : mc_rnodes) {
          systematics_processor_.bookSystematics(sample_key, rnode, binning,
                                                 model);
        }
      ...
      analysis::log::info("AnalysisRunner::run", "Region protocol complete (",
                          region_index, "/", region_count,
                          "):", region_handle.key_.str());
    }
