...
class AnalysisRunner {
  public:
    AnalysisRunner(AnalysisDataLoader &ldr, const SelectionRegistry &sel_reg,
                   ...
                   bool serialise_results = false,
                   std::string serialisation_dir = "variable_results")
        : analysis_definition_(sel_reg, var_reg), data_loader_(ldr),
          selection_registry_(sel_reg), systematics_processor_(sys_proc),
          histogram_booker_(std::move(booker)),
          serialise_results_(serialise_results),
    ...
    RegionAnalysisMap run() {
        ...
        plugin_manager.notifyInitialisation(analysis_definition_,
                                            selection_registry_);
        ...
        for (const auto &var_handle : analysis_definition_.variables()) {
            if (analysis_definition_.isDynamic(var_handle.key_)) {
                log::info("AnalysisRunner::run",
                          "Deriving dynamic bin schema for variable:",
                          var_handle.key_.str());

                std::vector<ROOT::RDF::RNode> mc_nodes;
                for (auto &[sample_key, sample_def] :
                     data_loader_.getSampleFrames()) {
                    if (sample_def.isMc()) {
                ...
                bool include_oob =
                    analysis_definition_.includeOutOfRangeBins(var_handle.key_);
                auto strategy = analysis_definition_.dynamicBinningStrategy(
                    var_handle.key_);
                BinningDefinition new_bins = DynamicBinning::calculate(
                    mc_nodes, var_handle.binning(), "nominal_event_weight",
                    400.0, include_oob, strategy);

                log::info(
                    "AnalysisRunner::run",
                    "--> Optimal bin count resolved:", new_bins.getBinNumber());

                analysis_definition_.setBinning(var_handle.key_,
                                                std::move(new_bins));
            }
        }
        ...
        for (const auto &region_handle : regions) {
            ++region_index;
            analysis::log::info("AnalysisRunner::run",
                                "Engaging region protocol (", region_index, "/",
                                region_count, "):", region_handle.key_.str());

            RegionAnalysis region_analysis =
                std::move(*region_handle.analysis());

            std::map<SampleKey, std::unique_ptr<ISampleProcessor>>
                sample_processors;
            ...
            for (auto &[sample_key, _] : sample_frames) {
                ...
                if (!run_config)
                    continue;
                if (!region_beam.empty() &&
                    run_config->beam_mode != region_beam)
                    continue;
            ...
            for (auto &[sample_key, sample_def] : sample_frames) {
                ...
                if (accounted_runs.insert(run_config->label()).second) {
                    region_analysis.setProtonsOnTarget(
                        region_analysis.protonsOnTarget() +
                        run_config->nominal_pot);
                }
                plugin_manager.notifyPreSampleProcessing(
                    sample_key, region_handle.key_, *run_config);
            ...
            for (const auto &var_key : vars) {
                ++var_index;
                const auto &variable_handle =
                    analysis_definition_.variable(var_key);
                ...
                analysis::log::info("AnalysisRunner::run",
                                    "Deploying variable pipeline (", var_index,
                                    "/", var_total, "):", var_key.str());
                analysis::log::info("AnalysisRunner::run",
                ...
                for (auto &[sample_key, rnode] : mc_rnodes) {
                    systematics_processor_.bookSystematics(sample_key, rnode,
                                                           binning, model);
                }
                ...
                analysis::log::info("AnalysisRunner::run",
                                    "Variable pipeline concluded (", var_index,
                                    "/", var_total, "):", var_key.str());
            }
            ...
            for (auto &[sample_key, _] : sample_processors) {
                plugin_manager.notifyPostSampleProcessing(
                    sample_key, region_handle.key_, analysis_regions);
            }

            analysis::log::info("AnalysisRunner::run",
                                "Region protocol complete (", region_index, "/",
                                region_count, "):", region_handle.key_.str());
        }
  ...
  private:
    void serialiseVariableResult(const RegionKey &region,
        ...
        auto write_hist = [&](const std::string &name,
                              const BinnedHistogram &hist) {
        ...
        for (const auto &[sample_key, var_map] : result.raw_detvar_hists_) {
            for (const auto &[variation, hist] : var_map) {
                write_hist("detvar_" + sample_key.str() + "_" +
        ...
        for (const auto &[key, vec] : result.universe_projected_hists_) {
            for (size_t i = 0; i < vec.size(); ++i) {
                write_hist("universe_" + key.str() + "_" +
                               std::to_string(i),
                           vec[i]);
            }
        }
