...
struct VariableOptions {
    ...
    bool load_intrinsic_strangeness_weights = false;
};
...
public:
    ...
    std::vector<std::string> GetVariables(
        ...
        AnalysisFramework::SampleType sample_type
    ) const;
...
private:
    ...
    std::vector<std::string> intrinsic_strangeness_weights_;
};
...
inline VariableManager::VariableManager() {
    ...
    intrinsic_strangeness_weights_ = {};
}

inline std::vector<std::string> VariableManager::GetVariables(
    const VariableOptions& options,
    AnalysisFramework::SampleType sample_type
) const {
    ...
    if (options.load_truth_event_info && AnalysisFramework::isSampleMC(sample_type)) {
    ...
    if (options.load_weights_and_systematics && AnalysisFramework::isSampleMC(sample_type) && !AnalysisFramework::isSampleDetVar(sample_type)) {
    ...
    if (options.load_intrinsic_strangeness_weights && AnalysisFramework::isSampleStrange(sample_type)) {
        vars_set.insert(intrinsic_strangeness_weights_.begin(), intrinsic_strangeness_weights_.end());
    }
