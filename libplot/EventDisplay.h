...
class EventDisplay {
public:
    EventDisplay(AnalysisDataLoader& loader,
                 int image_size,
...
private:
                   ...
                   const std::string* pdf_path)
    {
        ...
        std::string tag =
            plane + "_" + std::to_string(run_id) +
            "_" + std::to_string(subrun_id) +
            "_" + std::to_string(event_num);
        ...
        TCanvas canvas_sem(("csem_" + tag).c_str(), "", image_size_, image_size_/1.5);
        canvas_sem.cd();
        ...
        static const std::array<std::string, 10> names_ = {
            ...
            "ChargedKaon", "NeutralKaon", "Lambda",
            "ChargedSigma", "Other"
        };
