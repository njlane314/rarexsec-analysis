...
public:
  ...
  void visualiseEvent(const EventIdentifier &sample_event,
    ...
    auto det_u =
        df.Take<std::vector<float>>("event_detector_image_u").GetValue().at(0);
    auto det_v =
        df.Take<std::vector<float>>("event_detector_image_v").GetValue().at(0);
    auto det_w =
        df.Take<std::vector<float>>("event_detector_image_w").GetValue().at(0);
    ...
    for (auto const &plane : planes_) {
      plotPlane(run_id, subrun_id, event_num, plane, det_u, det_v, det_w, sem_u,
                sem_v, sem_w);
    }
  }
...
private:
  void plotPlane(int run_id, int subrun_id, int event_num,
    ...
    TCanvas canvas_det(("cdet_" + tag).c_str(), "", image_size_, image_size_);
  ...
  TH2F *makeDetectorHist(const std::string &tag,
    ...
    TH2F *h = new TH2F(tag.c_str(), tag.c_str(), image_size_, 0, image_size_,
  ...
  TH2F *makeSemanticHist(const std::string &tag, const std::vector<int> &data) {
